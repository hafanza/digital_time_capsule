...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _handleLogin() async {
    ...
    try {
      //KHUSUS UNTUK GOOGLE PLAY REVIEWER
      // Gunakan toLowerCase() agar tidak sensitif huruf besar/kecil (misal: @DemoAccount tetap bisa masuk)
      if (usernameInput.toLowerCase() == '@demoaccount' &&
          passwordInput == '12345678') {
        if (!mounted) return;
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => HomeScreen(username: usernameInput),
          ),
        );
        return;
      }
      ...
      if (userData == null) {
        _showWarning(
          "Akun tidak ditemukan! Silakan daftar dahulu.",
          Colors.orangeAccent,
        );
  ...
  void _showWarning(String pesan, Color warna) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          pesan,
          style: const TextStyle(fontFamily: 'VT323', fontSize: 16),
        ),
  ...
  InputDecoration _simpleInputDecoration({
